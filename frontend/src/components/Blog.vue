<template>
    <div class="container">
      ...
      <div v-else class="post-list">
        <div v-for="post in blogPosts" :key="post.id" class="blog-post">
          ...
          <div class="content-preview" v-html="renderMarkdown(post.content, 200)"></div>
        </div>
      </div>
    </div>
  </template>
  ...
  const renderMarkdown = (text, length = 200) => {
    const truncated = text.length > length ? text.substring(0, length) + '...' : text
    return DOMPurify.sanitize(marked.parse(truncated))
  }
  </script>
